...
public class UserManager {
     ...
     * @throws UserManagerException Thrown if the authentication failed or there was an error
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            } else if ("changePassword".equals(jc.getParsedCommand())) {
     ...
     * @param claims The <code>ClaimValue[]</code> array of profile claims (see <a
     * href="https://htrc3.pti.indiana.edu:9443/carbon/claim-mgt/claim-view.jsp?store=Internal&dialect=http://wso2.org/claims">available
     * claims</a>)
     * @param permissions The array of permission keys to assign to the user, for example:
     * "/permission/admin/login" (can be <code>null</code>)
...
//                ResourceActionPermission.GET.toString(), PermissionType.ALLOW.toString());
    ...
    /**
     * Get the list roles (if <code>userName == null</code>, then return all roles, otherwise return
     * user's roles)
     *
     ...
     *
     * @param filter The filter to use when retrieving the user names ("*" = retrieve all users;
     * "bo*" = retrieve all users whose user name starts with "bo")
    ...
    /**
     * Get the list of required user claims (expected to be supplied as part of the createUser
     * request)
     *
     ...
     *
     * @return A <code>Map&lt;String,String&gt;</code> of available role permissions, where the key
     * represents the permission key, and the value provides a human readable name for the
     * permission
     * @throws UserManagerException Thrown if an error occurred
     */
